...
class Tower(Axis):
    ...
    @cached_property
    def min_nm(self) -> int:
        return -(self._config.max_tower_height_mm + 5) * 1_000_000

    # FIXME: move to the config
    @cached_property
    def above_surface_nm(self) -> int:
        return -(self._config.max_tower_height_mm - 5) * 1_000_000
    ...
    # FIXME: move to the config
    @cached_property
    def calib_pos_nm(self) -> int:  # pylint: disable=no-self-use
        return 1_000_000

    # FIXME: move to the config
    @cached_property
    def resin_start_pos_nm(self) -> int:  # pylint: disable=no-self-use
        return 36_000_000

    # FIXME: move to the config
    @cached_property
    def resin_end_pos_nm(self) -> int:  # pylint: disable=no-self-use
        return 1_000_000
